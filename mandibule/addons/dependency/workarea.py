...
from mandibule.widgets.fields import TextField, BoolField
...
class DependencyFormLayout(GraphFormLayout):
    ...
    def __init__(self, workarea, id_=None):
        ...
        # Super first
        super(DependencyFormLayout, self).__init__(workarea, id_)
        # Then connect to signals
        table_modules_model.dataChanged.connect(
            lambda item: self.data_changed.emit())
        table_modules_model.dataChanged.connect(self._function_unsaved)
        ...
        table_models_model.dataChanged.connect(
            lambda item: self.data_changed.emit())
        table_models_model.dataChanged.connect(self._function_unsaved)
...
class DependencyWorkAera(GraphWorkArea):
    """Dependencies graph workarea."""
    __metadata__ = {
        'name': 'dependency',
    }

    def __init__(self, app, server_id, id_=None, new=False):
        GraphWorkArea.__init__(self, app, 'dependency', server_id, id_, new)
